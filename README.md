...
- **Lender names fold to one relationship** — every lender dropdown lists one option per real lender, not per spelling. `normalizeLenderName()` already folded case, punctuation and generic suffixes (`Associated` / `Associated Bank`, `First FInancial` / `First Financial Bank`, `Old National` / `Old National Bank`); the dropdowns now build on it via `foldLenderNames()` and label each option with the fullest spelling seen. Abbreviations no rule can derive from the spelling (`5/3` = Fifth Third, `BOKF` = Bank of Oklahoma, `FNBO` = First National Bank of Omaha) fold through the `LENDER_ALIASES` map in `src/lenderExposure.js` — add a pair there when a lender appears twice under two names. Folding applies to filtering and to the exposure rollup alike, so a bank's deals and dollars gather under one row however the deal was papered.

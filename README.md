...
- **Fed Funds Odds** *(new — existing layouts pick it up via **+ Add Widget**)* — CME FedWatch-style probabilities of a hike, cut, or hold at each upcoming FOMC meeting, computed two ways side by side so the sources can be compared: from **30-Day Fed Funds futures** (the input CME uses; the daily rate pull stores the ZQ strip from Yahoo Finance in `fed_funds_futures`, anchored on the NY Fed's effective fed funds rate and target range in `fed_funds_spot`) and from the **Chatham 1-Mo Term SOFR forward curve** the covenant tracker already prices from (sampled at each month start, anchored on overnight SOFR). Both run the same engine (`src/fedwatch.js`): each month's expected average rate is split at the meeting day into a before/after rate, the change is spread across the two nearest 25bp steps, and meetings chain into cumulative odds per target range. A side-by-side table shows each source's most likely range and probability-weighted move per meeting, and **Show month math** exposes the per-month decomposition. FOMC decision dates live in `FOMC_DECISION_DATES` (through 2027; append the next calendar when the Fed publishes it). **Setup:** run [`db/fedwatch_setup.sql`](db/fedwatch_setup.sql) once, then run the Daily Rate Pull workflow by hand to fill the tables.
...
|---|---|---|
| [`daily-curves.yml`](.github/workflows/daily-curves.yml) | Weekdays 22:47 UTC | Runs [`scripts/pull-curves.mjs`](scripts/pull-curves.mjs): stores the day's **10-Year Treasury yield** (treasury.gov, free) and **30-day Average SOFR** (NY Fed, free) into `rate_history`, plus — for the **Fed Funds Odds** widget, once `db/fedwatch_setup.sql` has run — EFFR / SOFR / the FOMC target range (NY Fed) into `fed_funds_spot` and the **30-Day Fed Funds futures** strip (Yahoo Finance, unofficial) into `fed_funds_futures` |
...
3. **Email digest (optional):** create a free [Resend](https://resend.com) account and add the `RESEND_API_KEY` repo secret — that one is a credential and stays a secret. Until a sending domain is verified in Resend, delivery only works to the Resend account owner's address; verify a domain and add a `TASK_EMAIL_FROM` secret (e.g. `Covenant Dashboard <alerts@yourdomain.com>`) for team-wide delivery. Without a key the nightly run still syncs tasks and just logs what it would have sent.
4. **Recipients — set them on the site, not in GitHub:** unlock editing on the Debt Dashboard, open the **Tasks & Reminders** widget and click **✉ Recipients**. Two lists: the **full digest** (everything in its reminder window) and the **accounting digest** (lender reporting deliverables only). They're saved company-wide in `settings.taskEmailRecipients` and picked up on the next nightly run — no repo access needed to change who gets reminded. The panel and the addresses only render in edit mode. `TASK_EMAIL_TO` / `TASK_EMAIL_ACCOUNTING_TO` secrets still work as a fallback for whichever list is left empty; an empty accounting list simply skips that send.

> The edit-mode gate hides addresses from the screen; it is not access control. Anyone signed in to the site can read the `settings` table through the API, so treat these as internal-but-not-secret. The Resend API key never leaves GitHub.

Each open task is emailed at most once every 7 days while it's inside its reminder lead window, and overdue items keep appearing until resolved — the two digests track that cool-down separately (`emailed_at` / `accounting_emailed_at`), so the team email never suppresses accounting's. Reporting deliverables default to a 21-day lead (per-requirement `lead_days`), so accounting sees an item three weeks before it's due; the deliverables themselves come from each loan's [reporting requirements](#loans), so an abstract imported without them generates no reminders — the Loans tab flags that case.
...
│   ├── parseReporting.js         # Loan-abstract reporting prose → dated, structured deliverables
│   ├── taskGen.js                # Reminder-task builders + email digest selection (nightly Action)
...
| `fed_funds_futures` | Daily 30-Day Fed Funds futures strip (one row per contract month per day) for the Fed Funds Odds widget | `db/fedwatch_setup.sql` |
| `fed_funds_spot` | Daily EFFR / SOFR / FOMC target range (NY Fed) anchoring the Fed Funds Odds widget | `db/fedwatch_setup.sql` |
...
| `loan_reporting_requirements` | Structured lender deliverables per loan (operating statements, guarantor financials, …) — parsed out of the abstract on import, edited in the Loans tab detail panel, expanded into dated reminders by the nightly task generator | `db/loan_reporting_setup.sql` |
| `deal_documents` | Per-loan document repository (loan agreement, guaranty, amendments, …) stored in the private `loan-docs` bucket under `docs/<loan_id>/`; timestamped paths make re-uploads new versions | `db/deal_documents_setup.sql` |
